...
#' @return A data frame with flight parameters calculated from elapsed seconds and count grouped by the mills of the flight mill.
#' @export

fm_params <- function(df, mill, elapsed_sec, counter, sector_distance, lowest_speed, min_time_interval, fm_duration)
  {
  df <- df %>%
    dplyr::group_by(mill) %>%
